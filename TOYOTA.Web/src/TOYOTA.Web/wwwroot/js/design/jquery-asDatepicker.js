...
(function ($, document, window, undefined) {
    ...
    Plugin.prototype = {
        ...
        _generateDaypicker: function (index) {
            ...
            this._generateHeader(index, this._date.currentYear[index] + '年 ' + (this._date.currentMonth[index] + 1) + '月');
        ...
        _generateDays: function (index) {
            ...
            //console.log(this.options.eventCustom);
            for (var j = 0; j < 42; j++) {
                ...
                if (j < daysFromPrevMonth) {
                    ...
                    var eventData = this.options.eventCustom;
                    if (eventData != undefined && eventData != null) {
                        for (var x = 0; x < eventData.length; x++) {
                            if (content == eventData[x].EachDate) {
                                clickornot = true;
                                if (eventData[x].Type == "S")
                                    className += ' ' + this.namespace + '_splan';
                                else
                                    className += ' ' + this.namespace + '_tplan';
                            }
                        }
                    }
                }
                ...
                if (clickornot)
                    html += '<span class="' + className + '" onclick=fn_OpenPopup(' + content + ')>' + content + '</span>';
                else
        ...
        _position: function () {
            var calendar_height = this.$picker.outerHeight(),
                ...
                input_top = this.$inputWrap.offset().top,// this.$el.offset().top,
                input_left = this.$inputWrap.offset().left,//this.$el.offset().left,
